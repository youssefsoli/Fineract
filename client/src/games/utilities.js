...
let baseLevels = {
  ...
  leftShoulderCount: 0,
  rightShoulder: 0,
  rightShoulderCount: 0,


}

const potentialPoints = ["leftShoulder", "rightShoulder", ]

/**
 * Detect when a user jumps
 */
export function findJumps(keypoints, minConfidence, ctx, width, scale = 1) {
  if (baseCount < 30) {
    for (let i = 0; i < keypoints.length; i++) {
      const keypoint = keypoints[i]
      ...
      } else if (keypoint.part === "leftFoot") {
  
      } else if (keypoint.part === "rightFoot") {
