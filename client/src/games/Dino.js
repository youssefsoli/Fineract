...
const Dino = ({ pose, canvasRef, ...props }) => {
    ...
    const draw = (ctx) => {
        ...
        const birdHeight = cvHeight * 0.1
        const birdWidth = birdHeight
        ...
        for(let i = game.obstacles.length-1; i >= 0; i--) {
            if (game.obstacles[i].type === "cactus") {
                ctx.drawImage(game.cactus, game.obstacles[i].x, game.obstacles[i].y, cactusHeight, cactusWidth);
            } else {
                ctx.drawImage(game.bird, game.obstacles[i].x, game.obstacles[i].y, birdHeight, birdWidth);
            }
        ...
        ctx.drawImage(dino.current, game.bX, cvHeight - dinoHeight, dinoHeight, dinoWidth);
    ...
    return (
        <>
            <img ref={bird} src="assets/bird.jpg" style={{display: 'none'}} className="hidden" alt="" />
            <img ref={bg} src="assets/background.png" style={{display: 'none'}} className="hidden" alt="" />
            <img ref={cactus} src="assets/cactus.png" style={{display: 'none'}} className="hidden" alt="" />
            <img ref={dino} src="assets/dino.png" style={{display: 'none'}} className="hidden" alt="" />
        </>
    );
};
